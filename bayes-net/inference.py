...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for eliminationVar in eliminationOrder:
            # factorlist = []
            # tempFactorsList = currentFactorsList
            # for factor in tempFactorsList:
            #     if eliminationVar in factor.conditionedVariables() or eliminationVar in factor.unconditionedVariables():
            #         factorlist.append(factor)
            #         currentFactorsList.remove(factor)
            # if len(factorlist)>1: 
            #     JoinFactor = joinFactors(factorlist)
            #     EliminateFactor = eliminate(JoinFactor, eliminationVar)
            ...
            # if str(type(joinedFactor))=="<class 'NoneType'>": continue
            if len(joinedFactor.unconditionedVariables()) == 1: continue
            EliminateFactor = eliminate(joinedFactor, eliminationVar)
            currentFactorsList.append(EliminateFactor)

        # if len(currentFactorsList) == 1: return currentFactorsList[0]
        # JoinFactor = joinFactors(currentFactorsList)
        # NewFactor1 = normalize(JoinFactor)

        if len(currentFactorsList) != 1:
            JoinFactor = joinFactors(currentFactorsList)
        else:
            JoinFactor = currentFactorsList[0]  
        NewFactor1 = normalize(JoinFactor)
        return NewFactor1
        "*** YOUR CODE END ***"
