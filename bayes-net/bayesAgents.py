...
def fillObsCPT(bayesNet, gameState):
    ...
    L = [(BOTTOM_LEFT_VAL, bottomLeftPos), (TOP_LEFT_VAL, topLeftPos), (BOTTOM_RIGHT_VAL, bottomRightPos), (TOP_RIGHT_VAL, topRightPos)]
    for (val, housePos) in L:
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in ObsFactor.getAllPossibleAssignmentDicts():
                ...
                if (food and not ghost) or (food and ghost):
                    if assignment[obsVar] == RED_OBS_VAL: prob = PROB_FOOD_RED
                    elif assignment[obsVar] == BLUE_OBS_VAL: prob = 1- PROB_FOOD_RED
                    elif assignment[obsVar] == NO_OBS_VAL: prob = 0
                elif not food and ghost:
                    if assignment[obsVar] == RED_OBS_VAL: prob = PROB_GHOST_RED
                    elif assignment[obsVar] == BLUE_OBS_VAL: prob = 1- PROB_GHOST_RED
                    elif assignment[obsVar] == NO_OBS_VAL: prob = 0
                elif not food and not ghost:
                    if assignment[obsVar] == NO_OBS_VAL: prob = 1
                    else: prob = 0
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for i in range(len(HOUSE_VALS)):
        prob = factor.getProbability(AssignmentDictList[i])
        if  prob > max_prob:
            max_assign = AssignmentDictList[i]
            max_prob = prob
    return max_assign
    "*** YOUR CODE END ***"
