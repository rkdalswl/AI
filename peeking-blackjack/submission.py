    ...
    def succAndProbReward(self, state, action):
        ...
        for i in state[2]:
            if i != 0:
                num += 1
        if action == 'Quit':
            succ.append(((state[0], None, None), 1, state[0]))
            return succ
        elif action == 'Peek':
            for idx, next_card in enumerate(state[2]):
                if next_card != 0:
                    succ.append(((state[0], idx, state[2]), 1/num, -self.peekCost))
            return succ
        elif action == 'Take':
            for idx, next_card in enumerate(state[2]):
                if ((next_card != 0) & (state[0] + self.cardValues[idx] <= self.threshold)):
                    pre_new_deck = list(state[2])
                    pre_new_deck[idx] -= 1
                    new_deck = tuple(pre_new_deck)
                    if num != 1:
                        succ.append((((state[0]+self.cardValues[idx]), None, new_deck), 1/num, 0))
                    elif num == 1:
                        succ.append((((state[0]+self.cardValues[idx]), None, None), 1, (state[0]+self.cardValues[idx])))
                elif ((next_card != 0) & (state[0] + self.cardValues[idx] > self.threshold)):
                    succ.append((((state[0]+self.cardValues[idx]), None, None), 1/num, 0))
            return succ
        # END_YOUR_ANSWER
...
class Qlearning(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, episode, isLast):
        ...
        max_q = self.getQ(newState, self.actions(state)[0])
        for a in self.actions(state):
            max_q = max(max_q, self.getQ(newState, a))
        sample = reward + self.discount*max_q
        diff = sample - self.getQ(state, action)
        for f, v in self.featureExtractor(state, action):
...
class SARSA(Qlearning):
    ...
    def incorporateFeedback(self, episode, isLast):
        ...
        # BEGIN_YOUR_ANSWER (our solution is 8 lines of code, but don't worry if you deviate from this)
        sample = reward + self.discount*self.getQ(newState, newAction)
        diff = sample - self.getQ(state, action)
        for f, v in self.featureExtractor(state, action):
            self.weights[f]=self.weights[f]+self.getStepSize()*diff*v
        # END_YOUR_ANSWER
...
def blackjackFeatureExtractor(state, action):
    ...
    # BEGIN_YOUR_ANSWER (our solution is 8 lines of code, but don't worry if you deviate from this)
    feature_list = []
    feature_list.append(((total, action), 1))
    if counts == None:
        return feature_list
    presence = ['total']
    for i in state[2]:
        if i != 0:
            ...
            presence.append(0)
    feature_list.append(((tuple(presence), action), 1))
    for i in enumerate(state[2]):
        feature_list.append(((i, action), 1))
    return feature_list
    # END_YOUR_ANSWER
