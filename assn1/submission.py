...
def create_nqueens_csp(n = 8):
    ...
    for i in range(1, n+1):
        csp.add_variable('x'+str(i), domain)
    for i in range(1, n):
        for j in range(i+1, n+1):
            csp.add_binary_factor('x'+str(i), 'x'+str(j), lambda x, y : x != y and abs(x-y)!=abs(i-j))
    # END_YOUR_ANSWER
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            MRV_count = self.csp.numVars+1
            for varr in self.csp.variables:
                if varr not in assignment:
                    RV_count = 0
                    for vall in self.domains[varr]:
                        RV_w = self.get_delta_weight(assignment, varr, vall)
                        if(RV_w > 0):
                            RV_count += 1
                    if MRV_count > RV_count:
                        MRV_count = RV_count
                        MRV = varr
            return MRV
            # END_YOUR_ANSWER
    ...
    def arc_consistency_check(self, var):
        ...
        queue = []
        for var2 in self.csp.get_neighbor_vars(var):
            queue.append((var2, var))
        for var1 in self.csp.variables:
            for val1 in self.domains[var1]:
                if self.csp.unaryFactors[var1] == 0:
                    self.domains[var1].remove(val1)
                    for var2 in self.csp.get_neighbor_vars(var1):
                        queue.append((var2, var1))
        while len(queue)!=0:
            x_tail, x_head = queue.pop(0)
            removed = False
            for val_tail in self.domains[x_tail][:]:
                consistency = False
                for val_head in self.domains[x_head]:
                    if self.csp.binaryFactors[x_tail][x_head][val_tail][val_head]!=0:
                        consistency = True
                if consistency == False:
                    self.domains[x_tail].remove(val_tail)
                    removed = True
            if removed == True:
                for x_neighbor in self.csp.get_neighbor_vars(x_tail):
                    queue.append((x_neighbor, x_tail))
        # END_YOUR_ANSWER
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    A_i_list = []
    for a in range(0, maxSum+1):
        for b in range(0, maxSum+1):
            if a+b <= maxSum:
                A_i_list.append((a,b))

    for i, X_i in enumerate(variables):
        A_i = ('sum', name, i)
        csp.add_variable(A_i, A_i_list)
        if i==0:
            def factor3(val, val2):
                a, b = val2
                return ((val == b) and (a== 0))
            csp.add_binary_factor(X_i, A_i, factor3)
        else:
            def factor1(val1, val2):
                a, b = val1
                c, d = val2
                return a+b == c
            csp.add_binary_factor(('sum', name, i - 1), A_i, factor1)
            def factor2(val, val2):
                c, d = val2
                return (val == d)
            csp.add_binary_factor(X_i, A_i, factor2)
    # consistency between A_n and result
    def factor3(val2, res):
        c, d = val2
        return res == c+d
    csp.add_binary_factor(A_i, result, factor3)
    return result
    # END_YOUR_ANSWER
...
def create_lightbulb_csp(buttonSets, numButtons):
    ...
    for t in range(numButtons):
        count = 0
        for i in range(numBulbs):
            if t in buttonSets[i]:
                count += 1
        csp.add_variable(t, [0, count])
    sumVar = get_sum_variable(csp, 'sum-up-to-', [i for i in range(numButtons)], numBulbs)
    csp.add_unary_factor(sumVar, lambda n: n == numBulbs)
    # END_YOUR_ANSWER
    return csp
